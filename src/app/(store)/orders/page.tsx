...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ring-1 ring-inset shrink-0 ${cfg.cls}`}>
...
function PaymentBadge({ status }: { status: string }) {
  if (status === 'paid') {
    return <span className="text-xs font-semibold text-success">Đã thanh toán</span>
  }
  if (status === 'cancelled') {
    return <span className="text-xs font-semibold text-error">Đã huỷ thanh toán</span>
  }
  return <span className="text-xs font-semibold text-gold">Chờ chuyển khoản</span>
}
